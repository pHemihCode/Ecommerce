...
function Register() {
  ...
  return (
    <div className='flex flex-col justify-center items-center mb-3 w-full px-2 relative top-36'>
        <form action="" onSubmit={handleSubmit(onSubmit)} className='bg-slate-200 p-3 rounded-md w-4/5 sm:w-3/5 sm:mb-8 lg:w-1/4'>
            <div className="input-grp text-md flex flex-col mb-1">
              <label htmlFor="fname" className='mb-1'>Full Name</label>
              <input type="text" id='fname' className='px-2 py-1 rounded-sm placeholder:italic outline-none' placeholder='Full name... ' {...register('fullname')}/>
              <i className='text-red-500 h-5 text-sm'>{errors.fullname?.message}</i>
            </div>
            <div className="input-grp text-md flex flex-col mb-1">
              <label htmlFor="phone" className='mb-1'>Phone Number</label>
              <input type="text" id='phone' className='px-2 py-1 rounded-sm placeholder:italic outline-none' placeholder='+234... ' {...register('phone')}/>
              <i className='text-red-500 h-5 text-sm'>{errors.phone?.message}</i>
            </div>
            <div className="input-grp text-md flex flex-col mb-1 ">
              <label htmlFor="email" className='mb-1'>Email</label>
              <input type="email" id='email' className='px-2 py-1 rounded-sm placeholder:italic outline-none' placeholder='abc@gmail.com' {...register('email')}/>
           ...
           <div className='flex flex-row justify-center'>
             <button className='bg-black text-md px-3 py-1 text-white my-3 rounded-sm'>Register</button>
           </div>
            <p className='text-md text-center'>You already have an account? <Link to='/login' className='!no-underline'>Login In</Link></p>
        </form>
    </div>
  )
}
