      ...
      <div className="fixed top-0 z-20 w-full bg-white h-28">
        <div className="bg-black text-center text-slate-200">
          <p className="italic py-2 text-sm">
            <FcFlashOn className="inline" /> FREE SHIPPING ON OTHERS MORE THAN
            $100 <FcFlashOn className="inline" />
          </p>
        </div>
          ...
          <div className="relative bottom-10 lg:bottom-0 lg:w-1/4">
            <div className="flex space-x-3 float-right text-2xl sm:pr-5 sm:text-3xl">
              ...
              >
                    ...
                    <Tooltip id="overlay-example" {...props}>
                      {theUser ? (
                        <div>
                          <div>
                            {theUser?.photoURL ? (
                              <img
                                ...
                                alt={theUser.displayName}
                                className="w-15 h-15 mx-auto my-3 rounded-full"
                              />
                            ) : (
                          ...
                          <button
                            onClick={SignOut}
                            className="border-2 w-24 mx-auto py-1 rounded-md mb-3 text-white"
                          >
                            Sign Out
                          </button>
                        </div>
                        ...
                        <div>
                          <button
                            ...
                            className="border-2 w-24 mx-auto py-1 rounded-md my-3 text-white"
                          >
                            Log In
                          </button>
                        </div>
                ...
                <p className="text-sm sm:text-md lg:text-md font-bold m-0 pr-1">
                  {theUser ? (
                    theUser.displayName ? theUser.displayName : theUser.email
                  ) : (
                    <Link>Log in</Link>
                  )}
                </p>
              ...
              <div className="relative">
                {cart.length > 0 && (
                  <span className="notifi absolute bg-red-500 w-3 right-0 sm:left-4 sm:bottom-6 h-3 bottom-5 rounded-full"></span>
                )}
                <Link to="/cart">
                  <FiShoppingCart />
                </Link>
